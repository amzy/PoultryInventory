...
class PoultryLog {
  ...
  factory PoultryLog.fromSheetRow(List<Object?> row) {
    ...
    // Supports the legacy sheet layout with Age in column B. Age is ignored
    // because it is always calculated from the configured flock start date.
    return PoultryLog(
  ...
  PoultryLog copyWith({DateTime? date, int? startingBirds, int? endingBirds, int? totalEggs, double? automatedFCR, double? layingPercentage, String? previousDateKey, int? previousEndingBirds}) => PoultryLog(
    date: date ?? this.date, startingBirds: startingBirds ?? this.startingBirds, mortality: mortality,
